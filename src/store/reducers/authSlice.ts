...
export const registerUser = createAsyncThunk('/auth/register', async (props: IDataRegister) => {
	const { data } = await axios.post(`${process.env.REACT_APP_API_URL}/auth/register`, props);
	return data;
})
export const loginUser = createAsyncThunk('/auth/login', async (props: IDataLogin) => {

	const { data } = await axios.post(`${process.env.REACT_APP_API_URL}/auth/login`, props);
	return data;
})
